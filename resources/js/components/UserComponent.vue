    ...
    >
                        ...
                        <v-form
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12">
                                            <v-text-field color="error" v-model="editedItem.name" label="Name" :rules="[rules.required, rules.min]"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row v-if="editedIndex == -1">
                                        <v-col cols="12" sm="12">
                                           <v-text-field type="password" color="error" v-model="editedItem.password" label="Type Password" :rules="[rules.required]"></v-text-field>
                                       </v-col>
                                        <v-col cols="12" sm="12">
                                           <v-text-field type="password" color="error" v-model="editedItem.rpassword" label="Retype Password" :rules="[rules.required, passwordMatch]"></v-text-field>
                                       </v-col>
                                        <v-col cols="12" sm="12">
                                           <v-text-field type="email" :success-messages="success" :error-messages="error" @:blur="checkEmail" color="error" v-model="editedItem.email" label="Email" :rules="[rules.required, rules.validEmail]"></v-text-field>
                                       </v-col>
                                    </v-row>
                                    <v-row>
                                       <v-col cols="12" sm="12">
                                           <v-select :items="roles" color="error" v-model="editedItem.role" label="Select Role" :rules="[rules.required]"></v-select>
                                       </v-col>
                                    </v-row>
                                </v-container>
                            </v-card-text>
        ...
        <template v-slot:item.role="{ item }">
            ...
            >
                ...
                <template v-slot:input>
                    <v-select :items="roles" color="error" v-model="item .role" label="Select Role" :rules="[rules.required]"></v-select>
                </template>
            </v-edit-dialog>
        </template>
        ...
        uploadPhoto(item) {
            if (this.editedItem.photo != null) {
                ...
                axios.post('/api/user/photo', formData)
                    ...
                    .catch(err => console.dir(error.response))
            }
        },
        updateRole(item) {
            const index = this.users.data.indexOf(item);
